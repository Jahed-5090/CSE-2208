/*
 * FALSE POSITION (REGULA FALSI) METHOD ALGORITHM
 * Same as Bisection Method else the middle point finding procedure.
 * Step 1: Choose 2 real numbers x1 and x2 such that f(x1) * f(x2) < 0,
 *         and define a stopping criterion E.
 * Step 2: Define the root x0 = x1 - [f(x1) * (x2 - x1)] / [f(x2) - f(x1)]
 * Step 3: Find f(x0).
...
void FALSE_POSITION_METHOD(double x1, double x2)
{
     double error = 1.0;
     int itr = 0;
